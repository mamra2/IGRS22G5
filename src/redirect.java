...
public class Redirect extends SipServlet {
   ...
   protected void doPublish(SipServletRequest request) throws IOException {
       ...
       stateDB.put(aor, new String(request.getRawContent(),
StandardCharsets.UTF_8).contains("<status><basic>open</basic></status>"));
   ...
   protected void doRegister(SipServletRequest request) throws IOException {
       ...
       String expiry = expiresHeader != null ? expiresHeader :
contactHeader.split("=")[1];
...
IOException, TooManyHopsException, ServletParseException {
       if (!inDomain(request)) {
           request.createResponse(403).send();
           return;
       }
...
ServletException, IOException {
       ...
       if (aorFrom.equals(GESTOR) && aorTo.equals(ALERTA) &&
registrarDB.containsKey(GESTOR)) {
           // 0 - command
           // 1 - argument
           ...
           switch (msg[0]) {
               ...
               case "CONF": {
                   ...
                   for (String c : colabDB) {
                       ...
                       res.setContent("NOT IMPLEMENTED".getBytes(),
"text/plain");
                       res.send();
                   }
                   request.createResponse(200).send();
